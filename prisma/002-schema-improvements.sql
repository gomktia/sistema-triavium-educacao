-- ============================================================
-- MELHORIAS NO SCHEMA (v2)
-- Fidelidade total aos documentos + robustez para produção
-- ============================================================

-- 1. TABELA DEDICADA: Itens do questionário VIA (71 itens)
--    Fonte: Escala 9 ano.xlsx - Questionário Socioemocional.pdf
--    Em vez de só JSON, ter referência fixa dos itens para validação
CREATE TABLE IF NOT EXISTS via_question_items (
  id          SERIAL PRIMARY KEY,
  item_number INTEGER NOT NULL UNIQUE,  -- 1 a 71
  text_pt     TEXT NOT NULL,            -- Texto em português
  strength    TEXT NOT NULL,            -- Nome da força associada
  virtue      TEXT NOT NULL,            -- Virtude (Sabedoria, Coragem, etc.)
  max_score   INTEGER NOT NULL DEFAULT 4  -- Escala 0-4
);

-- Inserir os 71 itens fielmente ao PDF "Questionário Socioemocional"
...
-- 2. TABELA DEDICADA: Itens SRSS-IE (12 itens)
--    Fonte: Protocolo SRSS-IE.pdf
CREATE TABLE IF NOT EXISTS srss_ie_items (
  id          SERIAL PRIMARY KEY,
  item_number INTEGER NOT NULL UNIQUE,  -- 1 a 12
  text_pt     TEXT NOT NULL,
  domain      TEXT NOT NULL CHECK (domain IN ('EXTERNALIZING', 'INTERNALIZING')),
  max_score   INTEGER NOT NULL DEFAULT 3  -- Escala 0-3
);
...
-- 3. TABELA: Pontos de corte SRSS-IE (referência do protocolo)
--    Fonte: Protocolo SRSS-IE.pdf - Seção 2
CREATE TABLE IF NOT EXISTS srss_ie_cutoffs (
  ...
  action_pt TEXT NOT NULL,  -- Ação recomendada em português
  UNIQUE(domain, tier)
);
...
('EXTERNALIZING', 'TIER_3', 9,  21, 'RED',    'Camada 3 (Intensiva): Encaminhamento individual para Psicólogo Escolar/Orientador. Necessidade de Plano Individual (PEI).'),
...
('INTERNALIZING', 'TIER_3', 6,  15, 'RED',    'Camada 3 (Intensiva): Encaminhamento individual para Psicólogo Escolar/Orientador. Necessidade de Plano Individual (PEI).')
ON CONFLICT (domain, tier) DO NOTHING;

-- 4. TABELA: Foco por série (RTI diferenciado)
--    Fonte: RTI Socioemocional.pdf - Seção 4 + Protocolo SRSS-IE - Seção 4
CREATE TABLE IF NOT EXISTS grade_focus_config (
  id             SERIAL PRIMARY KEY,
  grade          "GradeLevel" NOT NULL,
  focus_label    TEXT NOT NULL,
  srss_item      INTEGER NOT NULL REFERENCES srss_ie_items(item_number),
...
-- 2ª Série: Foco no Comportamento
...
-- 5. TABELA: Mapeamento VIA Força → Itens (24 forças)
--    Fonte: Escala 9 ano.xlsx - Aba "Avaliação" (fórmulas extraídas)
CREATE TABLE IF NOT EXISTS via_strength_mapping (
  id          SERIAL PRIMARY KEY,
  strength_id INTEGER NOT NULL,  -- 1 a 24
  strength    TEXT NOT NULL,
  virtue      TEXT NOT NULL,
  item_numbers INTEGER[] NOT NULL,  -- Array de itens que compõem a força
  max_possible INTEGER NOT NULL,     -- nº itens × 4
  UNIQUE(strength_id)
);
...
-- Transcendência
(20, 'Apreciação ao Belo',  'Transcendência',  ARRAY[39, 43],     8),   -- NOTA: apenas 2 itens
...
-- 6. RLS para as novas tabelas de referência (leitura pública, sem tenant)
...
-- Tabelas de referência: qualquer usuário autenticado pode ler
...
-- 7. CONSTRAINT de validação no JSON de rawAnswers
--    (PostgreSQL check com jsonb)
ALTER TABLE assessments ADD CONSTRAINT chk_raw_answers_not_empty
  CHECK (jsonb_typeof("rawAnswers") = 'object' AND "rawAnswers" != '{}'::jsonb);

-- 8. Policy adicional: Professor só pode criar SRSS_IE (não VIA)
CREATE POLICY teacher_srss_only ON assessments
