...
async function main() {
    console.log('Seeding FormQuestions...');

    // Prepare VIA Questions
    const questions = [];
    for (const [number, text] of Object.entries(VIA_ITEM_TEXTS)) {
        let category = null;

        // Find category (Virtue)
        for (const [virtueKey, virtueData] of Object.entries(VIA_ITEMS_BY_VIRTUE)) {
            if (virtueData.items.includes(Number(number))) {
                category = virtueData.label;
                break;
            }
        }
    ...
    console.log(`Seeded ${questions.length} VIA questions.`);
}
