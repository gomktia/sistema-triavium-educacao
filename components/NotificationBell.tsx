...
export function NotificationBell({ userId }: NotificationBellProps) {
    ...
    const { newCount, resetCount } = useRealtimeNotifications(userId ?? '');
    ...
    return (
        <Popover>
            <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" aria-label="Notificacoes" className="relative rounded-full hover:bg-slate-100">
                    ...
                    <span aria-live="polite" className="sr-only">
                        {totalUnread > 0 ? `${totalUnread} notificacoes nao lidas` : "Nenhuma notificacao nova"}
                    </span>
                </Button>
            </PopoverTrigger>
